...
def uniform_sample(vertices, faces, n_samples, with_normal=False):
    ...
    vec_cross = np.cross(faces[:, 1, :] - faces[:, 0, :],  # 法向量
                         faces[:, 2, :] - faces[:, 0, :])
...
def sample_points_from_mesh(path, n_pts, with_normal=False, fps=False, ratio=2):
    ...
    Args:
        ...
        fps(farthest_point_sampling): whether to use fps for post-processing, default False.
...
def load_depth(img_path):
    ...
    # CV2.IMREAD_UNCHANGED = -1, 读入一幅完整图片, 包括深度信息(alpha通道)
...
def compute_mAP(pred_results, out_dir, degree_thresholds=[180], shift_thresholds=[100],
    ...
    # 循环处理结果，收集iou和pose指标的pred匹配和gt匹配
    for progress, result in enumerate(tqdm(pred_results)):
        ...
        for cls_id in range(1, num_classes):
            ...
            # 计算 gt 实例 和 pred 实例的 iou(第 i 个物体在阈值为 s 的情况下匹配了 j 物体)
...
def plot_mAP(iou_aps, pose_aps, out_dir, iou_thres_list, degree_thres_list, shift_thres_list):
    ...
    plt.savefig(os.path.join(out_dir, "{:%Y.%m.%d_%H:%M:%S} mAP.png".format(now)))
    plt.close(fig)
    return
...
def translation_to_uv(translation, intrinsics, out_transform=None):
    origin = np.zeros(4)
    origin[3] = 1
    ...
    transed_origin = out_transform @ origin
    u, v, w = intrinsics @ transed_origin[:3]
    u_2d, v_2d = np.floor(u / w).astype(int), np.floor(v / w).astype(int)
    return u_2d, v_2d


def letterbox_image(image, size, mask=False, center=None):
    ...
    if mask:
        new_image = Image.new('RGB', size, (128, 128, 0))
    else:
