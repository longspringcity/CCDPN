...
def train():
    ...
    for i, data in enumerate(visual_dataloader):
        ...
        raw_rgb = raw_rgb[0].numpy().transpose(1, 2, 0)
        cv2.circle(raw_rgb, real_pos, 3, (255, 0, 0), cv2.FILLED)  # 真实值为蓝色
        cv2.circle(raw_rgb, pred_pos, 3, (255, 255, 0), cv2.FILLED)  # 预测值为黄色
